...
### Fixed
- CSV editor no longer truncates `YYYY-MM-DD` date cells to the year. `createCell` in `packages/extensions/csv-spreadsheet/src/utils/csvParser.ts` ran `parseFloat(trimmed)` to detect numbers before checking for date shapes. `parseFloat("2026-05-15")` returns `2026` (it stops at the first non-numeric character), so the cell was stored as `{ raw: "2026-05-15", computed: 2026 }`. `convertToGridSource` then preferred `cell.computed` when writing rows into the RevoGrid source, rendering `2026` instead of the original string. The file content on disk stayed correct because the CSV serializer reads `cell.raw`, not `cell.computed`; only the displayed value was wrong. New `ISO_DATE_PATTERN` regex guard before the `parseFloat` branch returns ISO-shaped strings as strings. Numeric strings like `2026` (a plain year) still take the numeric branch. Slash-delimited dates and other non-ISO formats are out of scope for this guard and fall under #329's remaining sub-bugs (currency/percent/Cmd+B/U/diff state, separate follow-ups). 14 new tests in `csvParser.createCell.test.ts`. (#329)
- Claude Code permission stream no longer dies on sessions with 15-25+ accumulated tracker tasks. `PROMPT_GRACE_MS` in `ClaudeCodeProvider.sendMessage` bumps from 5s to 30s. With heavy task accumulation, the binary's internal task-list `<system-reminder>` hook serializes slowly enough that the SDK `type: 'result'` chunk lands well before the hook finishes; the previous 5s grace timer then closed stdin while the hook was still working, the hook threw "stream closed" on its serializer write, the prompt controller transitioned to `ended`, and every subsequent permission-requiring tool (`Tool permission request failed: Error: Stream closed`) failed for the rest of the session. The reset-on-activity branch still resets the timer on every post-result chunk, so a turn that keeps streaming work never hits the timer. Idle / interrupted turns linger for the grace window before stdin closes; 30s is the trade-off for the task-heavy case. (#320)
- Pressing Enter at the end of a list item that ends with `\`inline code\`` followed by a space no longer keeps the new bullet in inline-code format. Lexical's `RangeSelection` carries `selection.format` forward when the cursor moves past inline text, and clearing the bit on the inline-code-then-space boundary is not part of the default chain. `selection.insertParagraph()` then seeded the new list item's first child with the stale code format, and `@lexical/list`'s node transform baked the format onto the new `ListItemNode` itself. New `ListEnterFormatClearPlugin` registers an `INSERT_PARAGRAPH_COMMAND` handler at `COMMAND_PRIORITY_NORMAL` (above `@lexical/list`'s `COMMAND_PRIORITY_LOW` handler) that clears `selection.format` when the cursor is collapsed at end-of-line inside a list item AND the immediately preceding character is whitespace (space, tab, or non-breaking space). Pure decision function `$clearFormatForListItemEnter` covered by 11 headless-editor tests; scoped to list items per the issue report. (#302)
- The "+" (Create new...) button in the session sidebar now works when all sessions are archived and the panel is empty. The dropdown JSX (New Session / New Worktree / New Terminal / New Blitz / etc.) only lived inside the main return of `SessionHistory`, so when the empty-state early-return fired (line 2422), clicking + opened the dropdown state (`newDropdownOpen=true`) but the JSX was never mounted and the user saw nothing happen. Extracted the dropdown into a `newDropdownPortal` JSX variable defined above the early-return check; both the empty-state return and the main return now reference it. The dropdown is `position: fixed` against the viewport so visual placement is identical from either return path. Reported by @TaiwanTammy. (#306)
- Cmd+O Quick Open and the @-mention picker now scope their "recent files" list to the current workspace. The `get-recent-workspace-files` IPC handler resolved the workspace from window state via `BrowserWindow.fromWebContents`; after the multi-project rail (#188) a single window can have several workspaces pinned, so the window-state path returned a stale workspace and Quick Open showed files from other pinned workspaces. The renderer now passes `workspacePath` explicitly; the handler uses the parameter when present and keeps the window-state fallback for backwards compatibility. The @-mention empty-query path also had a hardcoded `workspacePath + '/'` prefix filter that dropped every match on Windows where absolute paths use `\` as the separator; the filter is now platform-aware (accepts either `/` or `\`) and lives in a small `workspacePathPrefix.ts` helper with 7 unit tests covering POSIX, Windows, sibling-prefix false-positive guard, and trailing-separator edge cases. (#301, #304)
- Agent chat transcript find-in-page search bar no longer hides behind the session-phase pill on narrow widths. `FloatingTranscriptActions` (which renders the "Implementing" / "Planning" / etc. pill in the top-right corner of the transcript) was positioned `absolute top-1.5 right-3 z-[100]` and overlapped the sticky `TranscriptSearchBar`'s chevron / list / close buttons whenever the search bar was open. The runtime `RichTranscriptView` now emits a `onSearchBarVisibilityChange` callback as `showSearchBar` toggles, the parent `AgentTranscriptPanel` mirrors it into local state, and `FloatingTranscriptActions` accepts a `searchBarVisible` prop that shifts its `top` from `1.5` to `14` (about 56px, slightly more than the search bar's 44px height) with a 150ms transition. The pill remains accessible at its usual top-right position when the search bar is closed. (#309)
...
### Added
- Codex moved to the app-server transport for proper file-edit hooks and unified-diff capture. The SDK exec stream hooks system was inadequate and 0.129 changed the message formats; the new JSON-RPC v2 transport carries the full unified-diff text per change so reverse-applying hunks recovers starting state and enables full edit tracking. Introduces a `CodexProtocol` abstraction with `CodexSDKProtocol` (kept as the escape hatch) and the new `CodexAppServerProtocol`, toggleable via the `openaiCodex.transport` setting (default `app-server` in production). One codex app-server child per Nimbalyst session, reused across turns via an in-memory `ProtocolSession` cache; cleaned up on session deletion or stream error so child processes don't leak. Codex approval RPCs are routed through Nimbalyst's existing tool-permission system instead of `approvalPolicy: 'never'` blanket. New `CodexAppServerRawParser`; per-message dispatcher routes by `metadata.transport` so old SDK-mode sessions still render through the legacy `CodexRawParser` without bumping `CURRENT_VERSION`. MCP tool names use the canonical `mcp__server__tool` form on both transports so session-naming detection, AskUserQuestion routing, and widget routing see the same shape.
- `inheritModel` flag on the `spawn_session` MCP tool so `/launch-new-session` can keep the new session on the same model as the caller (e.g., opus stays on opus) instead of always falling back to the global default. Explicit `model` still wins over `inheritModel`. Documented `model` and `inheritModel` in the launch-new-session skill.
- LaTeX math rendering in agent transcript markdown. Adds `remark-math`, `rehype-katex`, and `katex` to render inline (`$...$`) and block (`$$...$$`) math notation in agent responses via `MarkdownRenderer`. (#136)
- Inline embeds for extension-edited files inside markdown documents. Paragraph-isolated markdown links to extension-edited files (Excalidraw canvas, mockup, datamodel, csv, sqlite) now render as live embeds with a chrome bar, resize handles, and a view/edit toggle that autosaves through the extension's `EditorHost`. A new `EmbedExtension` auto-upgrades qualifying links to an `EmbeddedFileNode`; Tab toggles between link and embed forms; `embed=false` title attr records explicit user opt-out so the auto-upgrade respects it. The embeddable extension set is driven by the pluggable custom-editor registry rather than a hardcoded list. All five built-in extension editors (excalidraw, mockuplm, datamodellm, csv-spreadsheet, sqlite-browser) honour reactive `host.readOnly` so view mode hides editing UI and edit mode writes back via `host.saveContent` on a 2s autosave cadence. Click-to-select gate uses an embed shield + isolation+inert so wheel events bubble past the embed to the host scroller until the user selects it (fixes Excalidraw eating trackpad scroll).
- Unified embedded editor path breadcrumbs so embed headers match the main editor breadcrumb toolbar, preserving folder and file navigation from embedded editors.
- Extension manifest now supports `supportsTranscriptEmbed` and `transcriptEmbedHeight` so any custom editor can opt into transcript embeds individually instead of the host hardcoding `.excalidraw`. The host (`SessionTranscript`) threads a `canEmbedFile` predicate through the `AgentTranscript` runtime so embeddability is decided from the `customEditorRegistry` without runtime importing electron code. `TranscriptEmbeddedFileCard` reads `supportsTranscriptEmbed` and the per-extension preview height from the registry and forces read-only. MockupLM (480px), DatamodelLM, and CSV Spreadsheet opt in alongside Excalidraw.
- Clear-all-unread action in the system tray to quickly clear unread AI sessions; tray-triggered read state syncs to open windows and tests.
- Workstream tag row collapses overflowing tags into a "+N" pill that opens a floating menu of the hidden tags instead of wrapping. The tag row stacks below the title for more horizontal room, with the provider icon vertically centered alongside.
- Human-readable widget for upstream Claude API service errors. When the Claude API returns a 5xx with `api_error` or `overloaded_error`, the new `ApiServiceErrorWidget` renders the error class, HTTP status + `error.type`, an action list (status.claude.com, retry, try another model, include `request_id`), and the `request_id` as a copy-friendly inline code span with a one-click copy button. A collapsed "Show raw error payload" disclosure preserves the original JSON for bug reports. Detection signals: explicit Anthropic error type token, any 5xx paired with a `req_...` correlation id, or the status-page domain mention paired with a 5xx. All three are intentionally narrow; non-matching messages still fall through to the generic error UI. (#277)
- Dev-only IPC `transcript:force-reparse-session` to drop and rewrite one session's canonical events, so parser fixes can be verified against existing sessions without bumping `CURRENT_VERSION`.

### Changed
- Codex session sync bandwidth reduced via in-place tool-payload truncation at sync time before encryption: Claude Code `tool_result` blocks and Codex `item.aggregated_output`/`output`/`result` strings are capped at 8 KB per block with an elision marker. The local raw log is untouched. A `SyncTruncationTracker` emits per-message and periodic rollup logs (raw vs. on-wire bytes, per-source breakdown, block-size histogram). `SessionRoom` TTL dropped from 30 to 14 days; the admin cleanup default for `PersonalSessionRoom` aligns with the alarm cutoff. Per-session 50 MB storage cap added with FIFO eviction (oldest 25% by sequence on overflow), checked after each append. `SessionRoom.alarm()` now uses `storage.deleteAll()` instead of `DELETE FROM` so TTL expiry actually reclaims SQLite pages. Admin cleanup endpoint and driver return rich storage stats (totals, eligible/purged bytes, size/age/message-count histograms, top-25 largest DOs, `updated_at` vs newest-message drift), accumulate across batches, and print interim snapshots every N batches plus on SIGINT.
- `MessageStreamingHandler` now tracks per-listener refs instead of `removeAllListeners`. The handler registered 14 listeners on the cached per-session provider using `removeAllListeners('<event>')` followed by `on('<event>', listener)`, which works while `handle()` is the sole subscriber but silently drops any other subscriber the moment another module attaches. New `installListener(provider, event, listener)` private method looks up the previously-installed listener for the same `(provider, event)` pair, removes only that one via `provider.off(event, prev)`, and installs the new one and records the ref in a `WeakMap<AIProvider, Map<string, Listener>>`. No behavioral change for current callers; subscriber resilience improves for future ones. (#225)
...
- Right-click Archive on a session in the session history sidebar now surfaces backend rejections as a visible error notification instead of failing silently. Four code paths in the renderer (`handleArchiveSession` and `handleArchiveMetaAgentSession` in `SessionHistory.tsx`, plus the auto-archive and confirm-archive paths in `useArchiveWorktreeDialog`) only logged failures to the console and ran their optimistic UI updates anyway. When the backend rejected the archive (worktree row missing, filesystem error renaming the worktree dir, DB write failure, session-not-found), the optimistic UI updated locally but the DB stayed unchanged, the session reappeared on the next refresh, and the user saw "Archive did nothing" with no feedback. All four paths now check `result.success === false` and show the rejection message via `errorNotificationService`. Behaviour on the happy path is unchanged. (#282 observability)
...
- OpenCode model picker now finds the user's `opencode.json` on Windows. `OpenCodeConfigService` was hardcoded to `~/.config/opencode/opencode.json` on every platform, but the `opencode-ai` Go binary itself writes to `%APPDATA%\opencode\opencode.json` on Windows by default. Configured providers (LM Studio bridge, Qwen3-Coder, Devstral, Kimi, DeepSeek, GLM, Mistral, GPT-OSS, Ollama, and any other entry under `provider.*`) were therefore invisible to the model picker because Nimbalyst was probing a path opencode-ai never authors on Windows. The service now mirrors opencode-ai's own search order: `$XDG_CONFIG_HOME/opencode/opencode.json` first, then the Windows-only `%APPDATA%\opencode\opencode.json`, then the cross-platform `~/.config/opencode/opencode.json` fallback. A new `getProbedPaths()` accessor and a `console.warn` from `OpenCodeProvider.getModels()` when the loader returns null make this diagnosable from `main.log` instead of failing silently. (#284)
- Codex usage indicator no longer sits on a stale percentage after the 5-hour or 7-day window resets. `CodexUsageService.extractRateLimitsFromEvent` was returning the most recent `token_count` event from the user's `~/.codex/sessions/*.jsonl` files whenever its `primary` block was non-null, with no check on `resets_at`. After the window reset the same JSONL line kept matching the scan-backward loop in `extractUsageSnapshotFromFile`, so the indicator stuck on the historical value (91% in the reporter's case) across panel refresh, app relaunch, and even machine restart - the cold-start `refresh()` re-read the same stale file each time. A new pure `filterRateLimitsByExpiry` helper drops each bucket independently when its `resets_at` Unix-seconds timestamp is at or before now: a still-active 7-day window keeps surfacing even when the 5-hour window just reset, and if both buckets are expired the scan continues into older lines / files for a still-active block. If nothing is active anywhere the snapshot falls through to `limitsAvailable: false` and the renderer shows `--` instead of the stale percentage. (#120)

- The PGLite lock file now self-heals after a main-process crash on Windows when the OS has reused the original lock holder's PID. `worker.js`'s lock liveness check used `process.kill(pid, 0)` and interpreted any `EPERM` error as "process still running." On Windows, `process.kill(pid, 0)` against a PID owned by another user / privilege level throws `EPERM` regardless of whether that PID is the original Nimbalyst process or a totally unrelated system process that recycled the number. AnisminC's report on #272: Nimbalyst PID 6732 crashed at 21:09Z, the user relaunched at 00:56Z (3h47m later, no reboot), Windows had recycled PID 6732 onto a system process, the EPERM-as-alive interpretation blocked the launch with `DATABASE_LOCKED`, and the only workaround was deleting the lock file manually. The lock liveness decision is now extracted to a pure `decideLockIsRunning` helper that adds a 60-second grace window inside the EPERM branch: a genuine competing Nimbalyst instance acquires its lock during the current launch attempt (always seconds-fresh), while a crash-stale lock has a timestamp from before the crash (minutes to hours old) and is correctly classified stale. (#272)
- The "Thinking..." indicator no longer stays pinned after the assistant finishes. `sessionStateListeners.ts` resolves an `ownedWorkspacePath` for each lifecycle event and returns early when it cannot be resolved. The null-guard added in commit `4e5dd9e7` (fixing #231 multi-project rail routing) silently dropped `session:completed` / `session:error` / `session:interrupted` events whose `workspacePath` was not on the event payload AND whose session was not yet in `sessionRegistryAtom`, typically a startup race or post-HMR re-evaluation. `sessionProcessingAtom` was never cleared and the spinner stayed on until the user hit Cancel. arcenik86 reported this on extended-thinking Opus sessions where slower turns make the race more likely. Terminal lifecycle events now clear `sessionProcessingAtom(sessionId)` and `sessionHasPendingInteractivePromptAtom(sessionId)` unconditionally before the workspace-routed branches (which still run when the path is known, preserving the safety-net reload and streaming-bucket cleanup). Fixes #116.
- Auto-update on macOS no longer fails with "The command is disabled and cannot be executed" after the download completes. `AutoUpdaterService.checkAndDownloadLatest` was calling `autoUpdater.checkForUpdates()` immediately before `downloadUpdate()` to "get the absolute latest version", but on macOS each `checkForUpdates()` spins up a new Squirrel.Mac proxy server and the new proxy tears down the prior one that Squirrel's `SQRLUpdater` was already holding a reference to. By the time `quitAndInstall` ran, the proxy was closed and Squirrel raised an NSException ("command is disabled"). The race is sensitive to process scheduling so adambhenry's arm64 hardware reproduced it more reliably than x86_64. Removed the redundant pre-download check so the single Squirrel session that fired `update-available` stays alive through `quitAndInstall`. Also added a `squirrel_install_disabled` branch to `classifyUpdateError` so the toast can distinguish this failure mode from the generic "unknown" bucket. Fixes #245.
- Codex `thread/resume` was only sending `{ threadId, cwd }`. Each resume spawns a fresh codex child, so without re-forwarding `mcp_servers`, `sandbox`, `approvalPolicy`, `developerInstructions`, `model`, and reasoning effort, the agent started with codex defaults and zero MCP tools attached. The agent honestly reported "the commit proposal tool is not available in the current request context" because it wasn't. `resumeSession` now reuses `buildThreadStartParams` and forwards everything except `ephemeral` (which the resume schema does not accept).
- Codex interactive widgets (`developer_git_commit_proposal`, `AskUserQuestion`) no longer deadlock. The widgets render off the `tool_call_started` canonical event, but the protocol and parser only produced events on `item/completed`, which fires after the MCP tool returns — yet the MCP tool blocks waiting for the user to click through the widget that the canonical event would have rendered. Both the live stream and the raw parser now emit a result-less `tool_call_started` on `item/started` for `mcpToolCall` items, with cross-message dedup so the later `item/completed` only adds the completed descriptor.
- Claude sessions preserved across project moves. Renames now use the shared Claude workspace path encoding, avoiding loss of native session directories for spaced paths.
- Unknown slash command failures in claude-code (turns producing only a result chunk with `num_turns 0`) no longer render as a completely blank turn. The parser gated the result-text backfill on `processedTextMessageIds` being empty and `suppressResultChunkText` being off — both wrong for later turns in a multi-turn batch or any resume batch. Both gates are now bypassed when `num_turns` is 0 since the result text is then the sole output of the turn and cannot duplicate anything.
- Codex chat attachments preserved in prompts and transcript. Codex document attachments now inline correctly for SDK and ACP sessions, and attachment metadata stays on Codex transcript user messages.
- Onboarding mode-picker no longer beach-balls indefinitely on cold start. 28 redundant `await import('../utils/store')` calls in the main process were converted to static imports (the module is already loaded statically; the dynamic re-imports were vestigial and violated the project's no-dynamic-imports rule). The renderer-side `onboarding:get` IPC now races against a 3s timeout in both `useOnboarding` and `UnifiedOnboarding`; on timeout the conservative path (show dialog, new-user defaults) runs so the picker stays interactive even if main is wedged. Timing for the `onboarding:get` handler (main) and both renderer call sites is now logged. (#260)
- Archived workstream children stay out of session lists.
- Archived worktrees stay out of session history. Archive recovery now archives lingering worktree sessions, archived worktree sessions and super loops are hidden from sidebar lists, and linked super-loop metadata is archived when a worktree is archived.
- Git commit failures surface instead of reporting fake success. Commit errors are now distinguished from user cancels so pre-commit hook rejections no longer hang the session in "thinking", hook stderr is preserved in the result so Claude and the widget can show why the commit failed, and one commit executor is shared across the IPC, auto-commit, and mobile paths so all three report failures consistently. Fixes #202.
- Copilot ACP retains context between turns. "Session is already loaded" from `session/load` is treated as success; previously every follow-up message silently fell back to `session/new`, discarding the conversation history kept in the live ACP process. The session is in fact ready for `session/prompt` when this error fires, so the existing session id is reused instead of creating a new one. Fixes #251.
- Wrap toggle in transcript code blocks now persists across `OverflowWrapper` remounts. The toggle was deselecting itself (sometimes immediately, sometimes after 30-60 seconds) because `OverflowWrapper.wordWrap` was local component state, and during streaming the parent `MarkdownRenderer` re-runs react-markdown on every content update, re-creating the children tree. When a code block's position shifted React reconciled it as a different instance, unmounted the old wrapper, and mounted a fresh one defaulting to `wordWrap = false`. A module-level `Map` (`wrapPreferenceByKey`, bounded LRU at 200) now survives remounts, keyed by `${messageId}:${node.position.start.offset}` so cross-block / cross-message bleed and the early-streaming hole that affected an earlier content-prefix-key attempt are both avoided. When the persist key cannot be derived (no messageId or AST node lacks position metadata), the wrapper falls back to a unique per-mount counter so each instance still gets its own cache slot. (#274)
- Detached HEAD state handled in the git extension. Detached HEAD is normalized in git IPC handlers and pull/push actions are disabled while detached.
- Git views refresh after `.gitignore` edits. Workspace and nested repo `.gitignore` filters reload live, git status caches invalidate, and status updates rebroadcast. Regression tests cover root and nested `.gitignore` changes.
- Session archive state propagated to iOS. `isArchived` is carried through the CollabV3 `metadata_updated` cache merge, new-entry, pending-update queue, and `applyPendingMetadataUpdates` paths so archive ops actually reach the server index. `SyncProvider.fetchIndex` returns `isArchived`; archived sessions are no longer filtered out of `getAllSessionsForSync` so the `SyncManager` comparison can detect them. Archive-state divergence is treated as sync-worthy even when local `updated_at` hasn't moved. For TTL-expired sessions that are locally archived, the index entry is pushed without messages so iOS can apply the archived flag and hide the row from its already-cached copy. iOS counts non-archived non-container sessions per project in `refreshAllProjectStats`/`refreshSessionCount` and recomputes after `projectBroadcast`; electron shows the same count in the session-history-filters bar.
- `claude-code` no longer shadows OAuth login with empty `ANTHROPIC_API_KEY`. The Claude native binary treats the mere presence of `ANTHROPIC_API_KEY` as a signal to use API-key auth even when the value is empty. Setting it to `''` as defense-in-depth shadowed valid OAuth/CLI logins, producing "Authentication failed" at prompt time while the Settings page still showed Claude as authenticated. The bootstrap-level delete in `main.ts` plus the per-source sanitization in `sdkOptionsBuilder` already prevent any inherited key from leaking through; the empty-string force-clear was a redundant layer that was actively breaking OAuth users. `config.apiKey` path is unchanged: explicit user-configured keys still win.
- Copilot CLI resolves from `%APPDATA%\npm` on packaged Windows. A packaged Nimbalyst on Windows launched from the Start menu only sees a minimal PATH that doesn't include `%APPDATA%\npm`, where `npm install -g @github/copilot` puts `copilot.cmd`. The provider's bare `copilot` fallback spawned nothing, so Copilot sessions failed even after a successful CLI login. New `resolveCopilotExecutableForRuntime()` walks the enhanced PATH and the standard npm-global locations on Windows (`%APPDATA%\npm\copilot.cmd`, plus `~\AppData\Roaming\npm` fallback) and the equivalents on Unix (`~/.local/bin`, `~/.npm-global/bin`, `/usr/local/bin`, `/opt/homebrew/bin`). Both `isCopilotInstalled()` and `configureProtocol()` route through it so the runtime check and the spawn path agree.
- Finder/Dock drag-and-drop into folders restored on Electron 32. Electron 32 removed `File.path`, so the file-tree's drop handler was silently no-oping on every external file. The path is now resolved via `webUtils.getPathForFile` in the preload and copy failures surface through the existing error dialog instead of being swallowed. Fixes #206.
- Finder drops on empty file-tree space copy into the workspace root. The root drop zone only recognized internal drags (`text/plain`); external Finder/Dock drags carry `Files` instead, so the highlight never appeared and the drop silently bailed. The dragover and drop handlers now resolve dropped files via `getPathForFile` and copy them into the workspace root. (NIM-584)
- Collab v3: image attachments migrated on Share to Team. Pasted images stored at `assets/<hash>.<ext>` on the originator's disk were left in place; collaborators (and the originator once viewing the doc in collab mode) saw broken images because `collab://...assets/...` is not a loadable URL. Share to Team now runs a pre-seed migration pass: open the collab doc to register the sender, scan the markdown for local image refs, upload each through the existing encrypted collab-asset path, rewrite the markdown to `collab-asset://` URIs, then seed the Y.Doc. New `markdownAssetScanner` (pure helpers: scan, resolve with workspace + path-traversal guard, rewrite via single regex pass) and `CollabAssetUploader` extracted from the upload-asset IPC. New `document-sync:migrate-local-assets` IPC, sender-authorized identically to upload-asset, with concurrency cap of 3 and structured per-ref results.
- Collab v3: duplicate session titles in `SessionRoom` removed. The client now encrypts title to `encryptedTitle`/`titleIv` before `updateMetadata`; the server strips any incoming plaintext title (defense-in-depth for older clients), stores ciphertext under `encrypted_title`/`title_iv`, and returns only the encrypted form from `getMetadata`.
...
### Added
- New `PromptForUserInput` MCP tool gives agents a single structured-prompt widget with five field types (`multiSelect`, `singleSelect`, `reorder`, `editText`, `confirm`) so a single prompt can collect several inputs at once instead of three narrow tools chained together. The inline `editText` field reuses the Lexical bundle already loaded in the desktop transcript and the iOS WKWebView, and `reorder` uses @dnd-kit with per-item removal honoring `minItems`. Wire-name is `PromptForUserInput` (not `RequestUserInput`) so it does not collide with the Codex CLI's built-in `request_user_input` tool that is gated to Plan mode. The schema is a flat type-discriminator object rather than `oneOf` because the OpenAI function-calling adapter Codex uses collapses `oneOf` fields. iOS round-trips the response through the existing transcript bridge. Voice forwarding computes a `voiceFriendly` hint client-side (not trusted from the agent) so voice defers to the screen for >6-item reorders or long `editText` drafts. Persists a synthetic `nimbalyst_tool_result` on settle so the canonical `tool_call` event picks up the answer even if the SDK subprocess exits before flushing its own `tool_result` block. iOS reorder uses `MouseSensor` + `TouchSensor` with a 200ms delay and suppressed selection callout so long-press starts a drag instead of letting iOS hijack the gesture into text selection.
...
- Allow renaming AI sessions and add a "Preferred Agent Language" global setting on the Agent Features panel. `update_session_meta` can now overwrite the session name (the prompt instructs the agent not to rename once named unless the user asks). The new language preference steers AI-generated session names toward the chosen language: `claude-code`'s out-of-band `generateSessionTitle` injects the language hint into its description (the SDK has no language parameter), and non-`claude-code` providers receive the same guidance through the session-naming prompt section. Fixes #219.
- Voice agent can run "Commit with AI" via a new `propose_commit` voice tool that runs the same Smart Commit flow as the button: `git:get-commit-context`, then `ai:sendMessage` with the canonical prefix so the Requesting Commit Proposal widget appears in the transcript. Voice approve/reject for `git_commit_proposal_request` is wired to the same `git:commit` + `messages:respond-to-prompt` calls the widget click already uses. Also fixes a bug where `exit_plan_mode_request` and `git_commit_proposal_request` pending prompts were stored with `promptType: 'ask_user_question_request'`, so voice users heard the generic "needs your input" fallback instead of the commit title or plan-mode explanation -- voice now reads the actual commit title and file count.
- Voice Mode settings panel surfaces microphone permission status with a deep link to System Settings. Shows a warning when the OS has not granted microphone access, with a button that deep-links to the macOS Privacy & Security > Microphone pane. Permission probing is gated to when voice mode is enabled so users who never opt in don't see the UI. Distinct copy for denied vs restricted vs not-determined states.
- Voice mode "Generate Project Summary" button no longer requires an Anthropic API key. The button now launches a new agent session in the user's configured agent (Claude Code, Codex, etc.) which writes `nimbalyst-local/voice-project-summary.md` using its own Write tool, then auto-navigates to the session so the user can watch it run. Confirmation dialog explains what's about to happen before launch; button is disabled with a link to AI Models when no agent is configured. Removes the hardcoded Anthropic fetch and the `voice-mode:generate-project-summary` IPC handler. Closes #201.
- Codex slash command autocomplete: workspace Codex skills are surfaced in the shared slash-command picker, with a safe built-in Codex command subset alongside Claude behavior. Slash discovery and autocomplete are unified across providers.
- Skills and commands compatibility layer for Claude Code and Codex: Claude Code commands run in Codex and vice versa. Workflow discovery is unified through one registry, and an extension manifest now supports `agentWorkflows` contributions.
- Map Codex reasoning items into transcript thinking blocks; raw Codex reasoning enabled in SDK config (latest models do not output it but support is in place).
- MCP servers configured with `command: "node"` now use Electron's bundled Node runtime, so MCP features work out of the box on fresh installs without a system-wide Node.js install. `MCPConfigService.processServerConfigForRuntime` rewrites bare `node` tokens (`node`, `node.exe`, case-insensitive) to `process.execPath` and adds `ELECTRON_RUN_AS_NODE=1` to the spawned env. Substitution only triggers inside Electron, so vitest, mobile builds, and CI behave unchanged. Absolute paths to user-pinned Node installs are preserved as-is, and other runtimes (`npx`, `npm`, `bun`, `deno`) are intentionally untouched. Refs #197.

### Changed
- Surface slow startup phases in user logs. Beach-ball reports gave no signal about which startup phase stalled because the heavy paths (PGLite init, project file sync, session index sync, tracker initial sync) were silent on the happy path. A new always-on `timeStartupPhase` helper logs `[StartupSlow] {name} took {ms}ms` when a phase crosses 2s, and the obvious culprits are now wrapped: PGLite init/repository init/backup service init/getStats, ProjectFileSync (scan, buildManifest, connect), SyncManager fetchIndex and getAllSessionsForSync, and TrackerSync initial sync.
- Upgrade bundled Codex SDK and harden Claude packaged binary resolution: reject unusable `app.asar` Claude binary paths in packaged builds, keep agent sessions on bundled or explicit custom binaries, and allow login flows and Codex detection to find real installed CLIs.

### Fixed
- Restore legacy plan and decision tracker docs on the Tracker board. Files saved with the older top-level `planStatus` / `decisionStatus` frontmatter were absent from the board because the import path treated them as bare documents and `resolveTrackerFrontmatter` only looked at `trackerStatus.type`. The Electron document service and the tracker resolver now read both legacy keys via the same `EXTENSION_OWNED_KEYS` table the detector already uses, so legacy planning files reappear on the board without a manual rewrite. Adds resolver unit coverage.
...
- Surface `automationStatus` documents as Tracker rows. `resolveTrackerFrontmatter` in `TrackerTable.tsx` was checking only `trackerStatus.type` and ignoring the `EXTENSION_OWNED_KEYS` map that `detectTrackerFromFrontmatter` already uses, so files created via the `/automation` command never appeared in the Tracker view despite being correctly classified everywhere else. Exports `EXTENSION_OWNED_KEYS` from `frontmatterUtils.ts` and adds an extension-owned-key lookup to the resolver before the canonical `trackerStatus.type` check, mirroring the pattern in `detectTrackerFromFrontmatter`. Fixes #67.
- Detect frontmatter on Windows files with CRLF line endings. A repo-wide audit found 13 frontmatter regex sites across the runtime, electron main, electron renderer, and automations extension that required bare `\n` after `---`, so files checked out with `git config core.autocrlf=true` were silently undetectable to the Tracker view, automations daemon, AI streaming context, AI tool frontmatter writes, diff plugin, frontmatter plugin field updates, the floating tracker-type helper, and the renderer's IPC and AI service paths. All 13 sites now use `\r?\n`. Sites that already tolerated CRLF (`FileHtmlExporter`, `frontmatterReader`, `CommandFileParser`, `ElectronDocumentService`, top-level `FrontmatterUtils`) are unchanged. Adds CRLF round-trip tests in both `packages/runtime` and `packages/extensions/automations`. Fixes #68.
- Show the tray icon on Linux and Windows. `TrayManager.initialize()` returned early on every non-macOS platform, so Linux AppImage and Windows builds had no system tray entry, even though the icon assets (`trayTemplate.png` / `trayTemplate@2x.png`) were already shipped via `extraResources` to all platforms. Removes the darwin-only guard, wraps the macOS-only `systemPreferences.subscribeNotification('AppleInterfaceThemeChangedNotification', ...)` call (and its unsubscribe) in a `process.platform === 'darwin'` check, and gates the macOS-only `tray.setTitle(...)` updates the same way. The cross-platform `nativeTheme.on('updated', ...)` listener stays in place on every OS. Adds three vitest cases that mock `process.platform` to verify Linux/Windows no longer bail out and macOS still subscribes the appearance notification. Fixes #39.
- Surface a user-visible error when deleting a file from the workspace tree fails. Reporter on Linux had a non-writable trash folder; the renderer logged `Failed to move item to trash` to the debug log but the UI gave no feedback after the user clicked through the delete confirmation. `FlatFileTree.handleDelete` and `handleDeleteMultiple` now open the existing `ErrorDialog` via `dialogRef.current?.open(DIALOG_IDS.ERROR, ...)` with the OS-level reason in `details`. Multiple-file deletes show a single summary dialog rather than one per failure. Fixes #195.
- Widen `ErrorDialog` `details` prop to accept a plain string in addition to the structured `DiffErrorDetails` shape. Previously the component dereferenced `details.replacements.length` directly in JSX, so any caller passing a string (e.g. an OS error message) crashed the render instead of showing the dialog. The string case now renders as a single preformatted block. Unblocks the delete-failure error dialog above (#216).
- Stop spamming `Update Error: net::ERR_NAME_NOT_RESOLVED` (and other transient network errors) on the auto-update background poll. Reporter on Windows was on a network that could not resolve the GitHub release feed, so every hourly check fired the error toast and the only escape was to dismiss it again on the next hour. `AutoUpdaterService` now reads the `error_type` from the existing `classifyUpdateError` helper and suppresses the user-facing `update-toast:error` when all three of: error stage is `check` (not download), error type is `network` (matches `enotfound`, `timeout`, `econnrefused`), and the check was the hourly auto-poll (not a manual `Check for Updates` click). The error is still logged via `log.error`, still reported to analytics under `update_error`, and still relayed to renderer subscribers via `update-error`. Manual checks still surface the toast so user-initiated clicks get feedback, and download errors still surface so mid-download failures are not swallowed. Fixes #56.
- Make `Ctrl+=` (without Shift) zoom in on Windows and Linux. The visible Zoom In menu item uses `Cmd+Plus`, which translates to the Electron accelerator `CmdOrCtrl+Plus`. On Windows / Linux QWERTY layouts the `+` character requires `Shift+=`, and the underlying Electron accelerator can also miss the keystroke when Windows reports the unshifted `=` keycode. Adds a hidden Zoom In menu item with `accelerator: 'CmdOrCtrl+='` so users can press the bare `=` key as well. Mirrors the hidden-accelerator pattern already used for `New File` in the same menu. Fixes #205.
- Add `Shift+=`, numpad `+`, and numpad `-` zoom accelerators as hidden menu items so all those keystrokes also trigger zoom (some keyboard drivers leave `Shift` set on the event for `=`). Completes the coverage started by #220. Refs #205.
- Honor `.gitignore` from nested git repos when watching a non-git workspace root. Workspaces containing nested git repos (e.g. a 4.7 GB rootfs build-output tree) could OOM the Electron main process because chokidar walked into ignored directories using only the workspace-root `.gitignore`. The watcher now caches `.gitignore` filters per discovered nested git root and consults the nearest enclosing one, memoizes git-root lookup per directory so the chokidar walk does at most one `existsSync` per unique ancestor, and the circuit-breaker error mentions nested `.gitignore`s. Fixes #207.
- Walk up to find the owning git root for nested repos so files committed inside a nested git repo no longer report as untracked indefinitely. `GitStatusService.getFileStatus` was bailing out whenever `workspacePath/.git` did not exist; now `findGitRootForFile(filePath, boundary)` walks up from the file's directory looking for a `.git` entry, stopping at the workspace boundary so files outside the workspace cannot resolve against an unrelated repo. Files are grouped by their owning root, `git status --porcelain` runs once per root with the right cwd, and paths are converted to root-relative form. Cache key now includes the sorted owning roots. Includes a follow-up fix to `clearCache` so null-byte-keyed cache entries (the new key shape) are matched by the invalidation predicate. Fixes #122.
- Read `customClaudeCodePath` from the `ai-settings` store in the path loader. The loader was calling `store.get()` against `app-settings.json`, but `AIService` writes the value to `ai-settings.json`, so the loader always returned an empty string and fell back to the bundled SDK binary instead of the configured custom executable. Moves the loader setup into the `AIService` constructor where it has direct access to the `ai-settings` store via `getSettingsStore()`. (#162)
- Initialize unified workstream state on drag-drop conversion in the agent view. Drag-dropping one session onto another (`skipSiblingCreation: true` path) was skipping `convertToWorkstreamAtom` entirely -- only the sibling-created path called it. `workstreamStateAtom(parentId)` was never written, leaving `childSessionIds=[]`, `type='single'` (default), and the dragged session's UI settings (layout mode, split ratio, files sidebar visibility) un-inherited onto the new parent. Subsequent reparent calls then operated on uninitialized state. `convertToWorkstreamAtom` now accepts `siblingId` as optional and the call site in `sessions.ts` always runs, so drag-drop conversions get the same full state initialization (including UI inheritance) the sibling-creation path has always had. (#212)
- Live-update sub-session renames and add Rename to the workstream parent menu. Renaming a sub-session via right-click did not live-update its title because `SessionHistory`'s `workstreamChildrenCache` only patches when its `renamedSession` prop reference changes, and nothing ever passed that prop -- the receiver effect was dead wiring. `AgentMode.handleSessionRename` now sets a `renamedSession` state and threads it through. Workstream parent rows also gained inline rename UI mirroring the existing worktree rename pattern, routing to the same `onSessionRename` callback the children already use. (#211)
- Tracker "+ Launch Session" now uses the workspace's default provider instead of hardcoded `claude-code`. Reporter on Codex-only installs hit "Provider claude-code is not enabled for this workspace" because `TrackerMainView.handleLaunchSession` called `aiCreateSession('claude-code', ...)` with the provider hardcoded, while `AgentMode.createNewSession` reads `defaultAgentModelAtom` and parses provider via `ModelIdentifier.tryParse(defaultModel)`. The tracker launch path now uses the modern `sessions:create` IPC and derives provider from `defaultAgentModelAtom`, falling back to `claude-code` only when no default model is set (fresh install). Fixes #176.
- Skip the hidden-editor mount for built-in file types in extension AI tool dispatch. The bridge called `ensureEditor` on any tool invocation that resolved a `filePath`, including filesystem-only tools whose active tab happened to be a built-in-editor file (e.g. a `.md` file owned by Lexical), throwing "No custom editor registered..." which surfaced as a hard tool failure. `ensureEditor` now returns early when no extension owns the file extension; the bridge already tolerates `editorAPI === undefined`, so filesystem-only tools (e.g. `automations.run`) execute normally while tools that genuinely need an editor API still surface their own meaningful error. Fixes #217.
- Hide the Environment Variables section from the Project tab in Settings. The panel reads/writes `~/.claude/settings.json`, which is global, so rendering it under the Project tab implied workspace-scoped state and silently leaked values across every project. Restricted to the User tab and tightened the help text to call out the global scope. Fixes #185.
- Persist the git extension panel's active tab and selected commit (by hash) per workspace, so reopening the panel lands on the same tab with the same commit selected. Selection survives commits being refetched and clears itself if the commit drops out of the visible list.
- Keep the git extension's Output log across panel close/open. The operation log moved from `useState` into a module-level store backed by `useSyncExternalStore` so history survives panel unmount/remount and the in-panel Refresh button. A renderer hard-reload still resets it.
- Prevent PGLite stampede on session list refresh. The `session_files` lookup is now bound to currently-uncommitted paths and concurrent session-list refreshes share one query instead of each scanning the workspace's full edit history. `getCachedSessionFiles` is replaced with `getSessionsForUncommittedFiles` (filters by `file_path = ANY(uncommittedFiles)` -- ~50 rows instead of thousands per call), and in-flight promise dedup is added to both `getCachedUncommittedFiles` and the new helper. A small `getSessionIdsWithEdits` helper still seeds zero counts for sessions whose files were just committed.
- Recover Shared Documents after laptop sleep/wake. Force-close the existing WebSocket on explicit reconnect requests even when it still reports `OPEN` -- post-wake the WS API can stay "connected" while the underlying transport is dead, leaving Shared Documents disconnected and blank until manual refresh. Drops the early-return on healthy/synced sockets in `DocumentSync`/`TeamSync`/`TrackerSync` `reconnectNow()` and `CollabV3Sync` `reconnectIndex()`; always tears down and reestablishes. Guards against the deferred close event from the torn-down socket via a `this.ws !== ws` identity check on every WS listener. Skips reconnect when an in-flight handshake is still `CONNECTING` so the post-wake event burst doesn't churn through half-finished sockets. Adds `TrackerSyncProvider` tests covering the zombie-OPEN reconnect, the stale-close race, and the `CONNECTING` burst guard.
- Stop read-only Codex bash commands (`sed -n`, `nl`, `cat`) from attributing as edits. A read-only bash command on a file with uncommitted modifications was producing a `session_files` "edited" row, leaking edits across sessions, because `trackBashEditsFromCommand` compared post-command disk content against a baseline that fell through to git `startSha` and `HEAD` on cache miss, so committed-vs-working-tree drift looked like a write the command never made. Now captures each Bash file's current disk content into `FileSnapshotCache` at `item.started` (before the command runs), adds `FileSnapshotCache.hasSnapshot` for tier-1-only checks, drops the `git show HEAD:` last-resort fallback, and preserves any in-flight pre-edit baseline from an earlier write in the same turn. Refs NIM-475.
- Make the Paste-as-Text shortcut reliable and visible. Normalizes Cmd/Ctrl+Shift+V detection in the AI input and exposes "Paste as Text" in the keyboard shortcuts dialog.
- Keep the voice mic open after the agent finishes a turn. Defers the 15s listen window until audio playback drains in the user's speakers (not when the server finishes streaming), wakes the mic from sleep at every turn end so function-call-only turns don't strand the user, raises the default listen window to 15s, and plays a short readiness cue when the mic wakes from sleep with no audible reply.
- Preserve inline markdown formatting after diff approval: bold spans containing inline code stay intact through approve-all round-trips. Adds a regression test.
- Restore shared mockup viewer rendering: prevent Electron-only share viewer crashes, bundle mockup viewer styles for standalone shared pages, and simplify shared mode to open directly in interactive view.
- Restore mockup diff review: re-enable AI diff mode for `.mockup.html` editors and prevent mockup edits from being auto-applied.

### Removed
- Remove unused `MCPConfigBuilder` from runtime. Never instantiated since it landed in the Feb 12 multi-platform refactor; providers route MCP config through `McpConfigService`, with an Electron-side override that handles platform command resolution. Drops the barrel re-export from `mcp/index.ts` and updates `design/agents/agent-provider-architecture.md` §7 and the file index to describe only the live `McpConfigService` path.
